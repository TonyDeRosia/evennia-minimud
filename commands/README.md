...
* `cweapon <name> <slot> <damage> [description]` - create a melee weapon in your
  inventory. The key always uses the capitalized name. A lowercase alias and a
  numbered alias such as `name-1` or `name-2` are added based on how many items
  share that key.

## Inspect Command

Use `inspect <item>` to view an object's details. The command accepts the
numbered aliases generated by `cweapon`, allowing you to inspect a specific item
